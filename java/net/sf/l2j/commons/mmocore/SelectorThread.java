...
public final class SelectorThread<T extends MMOClient<?>> extends Thread
{
	...
	public final void run()
	{
		...
		while (!_shutdown)
		{
			try
			{
				selectedKeysCount = _selector.selectNow(); // Pode substituir por select() para esperar de forma bloqueante
			}
			catch (IOException e)
			{
				e.printStackTrace();
			...
			if (selectedKeysCount > 0)
			{
				...
				while (selectedKeys.hasNext())
				{
					...
					selectedKeys.remove(); // Remove a chave para evitar processamento repetido
			...
			// Fechando as conexões pendentes
			...
			try
			{
				Thread.sleep(SLEEP_TIME); // Melhorar para ScheduledExecutorService se necessário
			}
			catch (InterruptedException e)
			{
				Thread.currentThread().interrupt(); // Garantir que o thread seja interrompido corretamente
				e.printStackTrace();
			}
		}
		closeSelectorThread(); // Fechar corretamente o Selector quando o loop acabar
	}
