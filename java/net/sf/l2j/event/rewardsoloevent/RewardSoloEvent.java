...
//	public static void bossSpawnMonster()
	public static void StartedEvent() // Iniciar Evento
	{
		...
		Broadcast.gameAnnounceToOnlinePlayers("[Reward Solo Event]: Duration: " + Config.EVENT_SOLO_FARM_TIME + " minute(s)!");
		_aborted = false;
		_started = true;
		
		waiter(Config.EVENT_SOLO_FARM_TIME * 60 * 1000);
		if (!_aborted)
		{
			Finish_Event();
		}
	...
	public static void Finish_Event()
	{
		...
		Broadcast.gameAnnounceToOnlinePlayers("[Reward Solo Event]: Finished!");
		Broadcast.gameAnnounceToAll("[Reward Solo Event]: Next Solo Event " + InitialSoloEvent.getInstance().getRestartNextTime() + " hours!");
		if (!AdminSoloEvent._bestfarm_manual)
		{
			InitialSoloEvent.getInstance().StartCalculationOfNextEventTime();
		}
		else
		{
			AdminSoloEvent._bestfarm_manual = false;
		}
	}
			...
			switch (seconds)
			{
				case 3600:
					if (_started)
					{
						...
						Broadcast.gameAnnounceToOnlinePlayers("[Reward Solo Event]: " + seconds / 60 / 60 + " hour(s) till event finish!");
					}
					break;
				case 60:
				case 120:
				...
				case 1800:
					if (_started)
					{
						...
						Broadcast.gameAnnounceToOnlinePlayers("[Reward Solo Event]: " + seconds / 60 + " minute(s) till event finish!");
					}
					break;
				case 1:
				case 2:
				...
				case 30:
					if (_started)
					{
						Broadcast.gameAnnounceToOnlinePlayers("[Reward Solo Event]: " + seconds + " second(s) till event finish!");
					}
					break;
			}
