...
public class NpcTable
{
	...
	private void load()
	{
		try
		{
			...
			hashFiles("npcs/Boss", files);
			hashFiles("npcs/EpicBoss", files);
									...
									if ("category".equalsIgnoreCase(dropCat.getNodeName()))
									{
										...
										for (Node item = dropCat.getFirstChild(); item != null; item = item.getNextSibling())
										{
											if ("drop".equalsIgnoreCase(item.getNodeName()))
											{
												...
												data.setEnchant(Integer.parseInt(attrs.getNamedItem("enchant").getNodeValue()));
												if (attrs.getNamedItem("enchantChance") != null)
												data.setEnchantChance(Integer.parseInt(attrs.getNamedItem("enchantChance").getNodeValue()));
												
												if (ItemTable.getInstance().getTemplate(data.getItemId()) == null)
												{
													_log.warning("Droplist data for undefined itemId: " + data.getItemId() +  " (Min: " + data.getMinDrop() + ", Max: " + data.getMaxDrop() + ") for NPC ID: " + npcId);
													continue;
												}
							...
							else if ("petdata".equalsIgnoreCase(cat.getNodeName()))
							{
							    ...
							    for (Node petCat = cat.getFirstChild(); petCat != null; petCat = petCat.getNextSibling())
							    {
							        if ("stat".equalsIgnoreCase(petCat.getNodeName()))
							        {
							            ...
							            // Multiplicador de status via Config
							            if (Config.ENABLE_CUSTOM_PET_MULTIPLIERS)
							            {
							                Double mult = Config.CUSTOM_PET_MULTIPLIERS.get(npcId);
							                if (mult != null && mult > 0)
							                {
							                    if (petSet.containsKey("pAtk")) petSet.set("pAtk", Double.parseDouble(petSet.getString("pAtk")) * mult);
							                    if (petSet.containsKey("mAtk")) petSet.set("mAtk", Double.parseDouble(petSet.getString("mAtk")) * mult);
							                    if (petSet.containsKey("pDef")) petSet.set("pDef", Double.parseDouble(petSet.getString("pDef")) * mult);
							                    if (petSet.containsKey("mDef")) petSet.set("mDef", Double.parseDouble(petSet.getString("mDef")) * mult);
							                    if (petSet.containsKey("hp"))   petSet.set("hp", Double.parseDouble(petSet.getString("hp")) * mult);
							                    if (petSet.containsKey("mp"))   petSet.set("mp", Double.parseDouble(petSet.getString("mp")) * mult);
							                    if (petSet.containsKey("hpRegen")) petSet.set("hpRegen", Float.parseFloat(petSet.getString("hpRegen")) * mult);
							                    if (petSet.containsKey("mpRegen")) petSet.set("mpRegen", Float.parseFloat(petSet.getString("mpRegen")) * mult);
							                }
							            }
