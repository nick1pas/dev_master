...
public final class TaskManager
{
	...
	{
		registerTask(new TaskClanRankingReward());
	}
...
//	}
	public static boolean addUniqueTask(String task, TaskTypes type, String param1, String param2, String param3, long lastActivation) {
	    try (Connection con = ConnectionPool.getConnection()) {
	        // Primeiro PreparedStatement
	        try (PreparedStatement statement = con.prepareStatement(SQL_STATEMENTS[2])) {
	            statement.setString(1, task);
	            try (ResultSet rset = statement.executeQuery()) {
	                if (!rset.next()) {
	                    // Segundo PreparedStatement, somente se o primeiro não encontrar nada
	                    try (PreparedStatement insertStatement = con.prepareStatement(SQL_STATEMENTS[3])) {
	                        insertStatement.setString(1, task);
	                        insertStatement.setString(2, type.toString());
	                        insertStatement.setLong(3, lastActivation);
	                        insertStatement.setString(4, param1);
	                        insertStatement.setString(5, param2);
	                        insertStatement.setString(6, param3);
