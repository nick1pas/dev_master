...
public final class PvpFlagTaskManager implements Runnable
{
	...
	public final void run()
	{
		...
		for (Map.Entry<Player, Long> entry : _players.entrySet())
		{
			...
			if (player.isInsideZone(ZoneId.TIME_FARM_ZONE) || player.isInsideZone(ZoneId.FLAG) || player.isInsideZone(ZoneId.FLAG_AREA_BOSS) || player.isInsideZone(ZoneId.RAID_ZONE) || player.isInsideZone(ZoneId.PVP_CUSTOM) || player.isInsideZone(ZoneId.FARM_HWID_FLAG))
			{
				_players.remove(player);
				continue;
			}
