...
public class AdminEffects implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		{
			for (Player player : activeChar.getKnownList().getKnownType(Player.class))
			{
				if (!player.isGM())
				{
		...
		else if (command.startsWith("admin_changename"))
		{
			try
			{
				...
				if (player instanceof Player)
					L2World.getInstance().removePlayer((Player) player);
				...
				if (player instanceof Player)
				{
					...
					((Player) player).broadcastUserInfo();
				}
		...
		else if (command.startsWith("admin_social"))
		{
			try
			{
				...
				if (st.countTokens() == 2)
				{
					...
					{
						Player player = L2World.getInstance().getPlayer(targetOrRadius);
						if (player != null)
						{
						...
						else
						{
							...
							for (L2Object object : activeChar.getKnownList().getKnownTypeInRadius(Creature.class, radius))
	...
	private static boolean performSocial(int action, L2Object target)
	{
		if (target instanceof Creature)
		{
			if (target instanceof L2Summon || target instanceof L2ChestInstance || (target instanceof L2Npc && (action < 1 || action > 3)) || (target instanceof Player && (action < 2 || action > 16)))
				return false;
