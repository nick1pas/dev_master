...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.startsWith("admin_character_info"))
		{
			try
			{
				final Player target = L2World.getInstance().getPlayer(command.substring(21));
				if (target != null)
		...
		else if (command.startsWith("admin_summon_info"))
		{
			...
			{
				L2Summon pet = ((Player) target).getPet();
				if (pet != null)
				{
		...
		else if (command.startsWith("admin_clan_info"))
		{
			try
			{
				final Player player = L2World.getInstance().getPlayer(command.substring(16));
				if (player == null)
				{
		...
		else if (command.startsWith("admin_remove_clan_penalty"))
		{
			try
			{
				...
				Player player = L2World.getInstance().getPlayer(playerName);
				if (player == null)
				{
...
//		}
	...
	private static void gatherSummonInfo(L2Summon target, Player activeChar)
	{
	...
	private static void gatherPartyInfo(Player target, Player activeChar)
	{
		final StringBuilder sb = new StringBuilder(400);
		for (Player member : target.getParty().getPartyMembers())
		{
