...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	protected boolean autoAttackCondition(Creature target)
	{
		...
		if (target instanceof Playable)
		{
			...
			if (targetPlayer != null)
			{
				...
				if (targetPlayer.isGM() || targetPlayer.isGM() && (targetPlayer.getAppearance().getInvisible() || !targetPlayer.getAccessLevel().canTakeAggro()))
					return false;
				
				// Check if player is an allied Varka.
				if (Util.contains(me.getTemplate().getClans(), "varka_silenos_clan") && targetPlayer.isAlliedWithVarka())
					return false;
				
				// Check if player is an allied Ketra.
				if (Util.contains(me.getTemplate().getClans(), "ketra_orc_clan") && targetPlayer.isAlliedWithKetra())
					return false;
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		if (intention == CtrlIntention.IDLE || intention == CtrlIntention.ACTIVE)
		{
			...
			if (!npc.isAlikeDead())
			{
				...
				if (!npc.getKnownList().getKnownType(Player.class).isEmpty())
	...
	protected void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			for (Creature target : npc.getKnownList().getKnownType(Creature.class))
			{
				...
				if (npc instanceof L2FestivalMonsterInstance && target instanceof Player)
				{
	...
	protected void thinkAttack()
	{
		...
		if (attackTarget == null || _attackTimeout < System.currentTimeMillis() || Util.calculateDistance(npc, attackTarget, true) > 2000)
		{
			...
			if (!defaultList.isEmpty())
			{
				...
				for (L2Skill sk : defaultList)
				{
					...
					if (sk.getTargetType() == SkillTargetType.TARGET_ONE)
					{
						...
						for (Attackable obj : npc.getKnownList().getKnownTypeInRadius(Attackable.class, sk.getCastRange() + actorCollision))
						{
							if (obj.isDead())
								continue;
							
							if (!Util.contains(actorClans, obj.getTemplate().getClans()))
								continue;
		...
		if (Rnd.get(100) <= 3)
		{
			...
			{
				if (nearby instanceof Attackable && npc.isInsideRadius(nearby, actorCollision, false, false) && nearby != attackTarget)
				{
	...
	protected boolean cast(L2Skill sk, double distance, int range)
	{
		...
		switch (sk.getSkillType())
		{
			...
			case BALANCE_LIFE:
			{
				...
				if (sk.getTargetType() == SkillTargetType.TARGET_ONE)
				{
					for (Attackable obj : caster.getKnownList().getKnownTypeInRadius(Attackable.class, (int) (sk.getCastRange() + caster.getCollisionRadius())))
					{
						if (obj.isDead())
							continue;
						
						if (!Util.contains(caster.getTemplate().getClans(), obj.getTemplate().getClans()))
							continue;
				...
				if (sk.getTargetType() == SkillTargetType.TARGET_PARTY)
				{
					for (Attackable obj : caster.getKnownList().getKnownTypeInRadius(Attackable.class, (int) (sk.getSkillRadius() + caster.getCollisionRadius())))
					{
						if (!Util.contains(caster.getTemplate().getClans(), obj.getTemplate().getClans()))
							continue;
	...
	protected Creature targetReconsider(int range, boolean rangeCheck)
	{
		...
		{
			for (Creature target : actor.getKnownList().getKnownTypeInRadius(Creature.class, actor.getTemplate().getAggroRange()))
			{
	...
	protected void onEvtAttacked(Creature attacker)
	{
		...
		if (attacker != null)
		{
			...
			{
				for (Attackable called : me.getKnownList().getKnownTypeInRadius(Attackable.class, me.getTemplate().getClanRange()))
				{
					...
					if (!Util.contains(actorClans, called.getTemplate().getClans()))
						continue;
					
					// Called mob doesnt care about that type of caller id (the bitch !).
					if (Util.contains(called.getTemplate().getIgnoredIds(), me.getNpcId()))
						continue;
					...
					if ((calledIntention == CtrlIntention.IDLE || calledIntention == CtrlIntention.ACTIVE || (calledIntention == CtrlIntention.MOVE_TO && !called.isRunning())) && GeoEngine.getInstance().canSeeTarget(me, called))
					{
						if (attacker instanceof Playable)
						{
							...
							if (quests != null)
							{
								...
								boolean isSummon = attacker instanceof L2Summon;
				...
				if ((calledIntention == CtrlIntention.IDLE || calledIntention == CtrlIntention.ACTIVE || (calledIntention == CtrlIntention.MOVE_TO && !called.isRunning())) && GeoEngine.getInstance().canSeeTarget(me, called))
				{
					if (target instanceof Playable)
					{
						...
						if (quests != null)
						{
							...
							boolean isSummon = target instanceof L2Summon;
							for (Quest quest : quests)
								quest.notifyFactionCall(called, me, player, isSummon);
						}
					}
	...
	private boolean checkBuffAndSetBackTarget(L2Object target)
	{
