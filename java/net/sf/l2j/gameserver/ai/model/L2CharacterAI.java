...
{
	public L2CharacterAI(Creature character)
	{
		super(character);
	}
...
//		}
	...
	public boolean canAura(L2Skill sk)
	{
		...
		{
			for (L2Object target : _actor.getKnownList().getKnownTypeInRadius(Creature.class, sk.getSkillRadius()))
			{
				if (target == getTarget())
					return true;
			}
	...
	public boolean canAOE(L2Skill sk)
	{
		if (sk.getSkillType() != L2SkillType.NEGATE || sk.getSkillType() != L2SkillType.CANCEL)
		{
			if (sk.getTargetType() == L2Skill.SkillTargetType.TARGET_AURA || sk.getTargetType() == L2Skill.SkillTargetType.TARGET_BEHIND_AURA || sk.getTargetType() == L2Skill.SkillTargetType.TARGET_FRONT_AURA)
			{
				...
				for (Creature target : _actor.getKnownList().getKnownTypeInRadius(Creature.class, sk.getSkillRadius()))
				{
			...
			{
				boolean cancast = true;
				for (Creature target : ((Creature) getTarget()).getKnownList().getKnownTypeInRadius(Creature.class, sk.getSkillRadius()))
				{
					if (!GeoEngine.getInstance().canSeeTarget(_actor, target))
						continue;
	...
	public boolean canParty(L2Skill sk)
	{
		...
		for (Attackable target : _actor.getKnownList().getKnownTypeInRadius(Attackable.class, sk.getSkillRadius()))
		{
			if (!GeoEngine.getInstance().canSeeTarget(_actor, target))
				continue;
