...
public class L2SiegeGuardAI extends L2AttackableAI
{
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		...
		if (intention == CtrlIntention.IDLE)
		{
			...
			if (!_actor.isAlikeDead())
			{
				...
				if (!getActiveChar().getKnownList().getKnownType(Player.class).isEmpty())
	...
	protected void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			for (Creature target : npc.getKnownList().getKnownTypeInRadius(Creature.class, npc.getTemplate().getClanRange()))
			{
	...
	protected void thinkAttack()
	{
		...
		if (willCastASpell())
		{
			...
			if (!defaultList.isEmpty())
			{
				...
				for (Creature cha : actor.getKnownList().getKnownTypeInRadius(Creature.class, 1000))
				{
					...
					if (!actor.isAttackingDisabled() && (cha instanceof Player && actor.getCastle().getSiege().checkIsDefender(((Player) cha).getClan())) || (cha instanceof L2Npc && Util.contains(clans, ((L2Npc) cha).getTemplate().getClans())))
					{
		...
		if (Rnd.get(100) <= 3)
		{
			...
			{
				if (nearby instanceof Attackable && actor.isInsideRadius(nearby, actorCollision, false, false) && nearby != attackTarget)
				{
