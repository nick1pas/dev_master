...
import net.sf.l2j.dolls.DollHolder;
...
import net.sf.l2j.gameserver.balance.matchup.MatchupProfile;
import net.sf.l2j.gameserver.balance.skills.SkillBalance;
...
public final class Formulas
{
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss)
	{
		...
		if (attacker instanceof Player && target instanceof Player)
		{
			...
			if (!player.isInOlympiadMode() && !enemy.isInOlympiadMode())
			{
				DollHolder doll = DollsTable.getRune(player);
				
				if (doll != null)
				{
					double skillPower = doll.getSkillPower();
					damage *= skillPower;
				}
			}
		}
		
		SkillBalance bal = SkillBalanceHolder.get(skill.getId());
		if (bal != null)
			damage *= bal.getDouble("powerMul", 1.0);
		...
		if (attacker instanceof Player && target instanceof Player)
		{
			...
			MatchupProfile matchup = MatchupHolder.get(player.getClassId().getId(), enemy.getClassId().getId());
			
			if (matchup != null)
			{
				damage *= matchup.getMul("blowDamageMul");
			}
		}
	...
	public static final double calcPhysDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
		...
		double defence = target.getPDef(attacker);
		ClassProfile profile = ClassProfileHolder.getProfile(attacker);
		...
		if (profile != null)
		{
			damage *= profile.getDouble("pAtkMul", 1.0);
		}
		...
		// CRITICAL FORMULA — permanece intacta
		if (crit)
		{
			...
			if (profile != null)
			{
				damage *= profile.getDouble("critRateMul", 1.0);
			}
		...
		if (isPvP)
		{
			damage *= attacker.calcStat(skill == null ? Stats.PVP_PHYSICAL_DMG : Stats.PVP_PHYS_SKILL_DMG, 1, null, null);
		}
		...
		if (attacker instanceof Player && target instanceof Player)
		{
			...
			if (matchup != null)
			{
				damage *= matchup.getMul("skillDamageMul");
			}
		}
	...
	public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit)
	{
		...
		{
			damage *= attacker.calcStat(skill.isMagic() ? Stats.PVP_MAGICAL_DMG : Stats.PVP_PHYS_SKILL_DMG, 1, null, null);
		}
		
		ClassProfile profile = ClassProfileHolder.getProfile(attacker);
		if (profile != null)
		{
			damage *= profile.getDouble("mAtkMul", 1.0);
		}
		
		damage *= calcElemental(attacker, target, skill);
		
		// Dolls (fora da Olympiad)
		if (attacker instanceof Player)
		{
			Player player = (Player) attacker;
	...
	public static final double calcMagicDam(L2CubicInstance attacker, Creature target, L2Skill skill, boolean mcrit, byte shld)
	{
	...
	public static final double calcLethal(Creature activeChar, Creature target, int baseLethal, int magiclvl)
	{
		...
		{
			Player atk = (Player) activeChar;
			Player tgt = (Player) target;
			
			MatchupProfile matchup = MatchupHolder.get(atk.getClassId().getId(), tgt.getClassId().getId());
			
			if (matchup != null)
			{
				chance *= matchup.getMul("lethalRateMul");
			}
		}
	...
	public static final void calcLethalHit(Creature activeChar, Creature target, L2Skill skill)
	{
		...
		if (target instanceof L2Npc && Config.LETHAL_MONSTERS_ID.contains(((L2Npc) target).getNpcId()))
			return;
	...
	public static boolean calcEffectSuccess(Creature attacker, Creature target, EffectTemplate effect, L2Skill skill, byte shld, boolean bss)
	{
		...
		if (Config.ENABLE_CUSTOM_CHANCE_SKILL && !(target instanceof L2MonsterInstance))
		{
		...
		else if (Config.SKILL_MAX_CHANCE && Config.SKILL_LIST_SUCCESS.contains(skill.getId()) && !(target instanceof L2MonsterInstance))
		{
	...
	public static boolean calcCubicSkillSuccess(L2CubicInstance attacker, Creature target, L2Skill skill, byte shld, boolean bss)
	{
	...
	public static double calcManaDam(Creature attacker, Creature target, L2Skill skill, boolean ss, boolean bss)
	{
		...
		double mAtk = attacker.getMAtk(target, skill);
		double mDef = target.getMDef(attacker, skill);
		double mp = Math.max(1, target.getMaxMp());
		
		// Multiplicador de classe (MAtk)
		if (profile != null)
			mAtk *= profile.getDouble("mAtkMul", 1.0);
		...
		// Dano base (estilo L2OFF / IL)
		double damage = (Math.sqrt(mAtk) * skill.getPower(attacker) * (mp / 97.0)) / mDef;
		
		// Vulnerabilidades
		damage *= calcSkillVulnerability(attacker, target, skill, skill.getSkillType());
		
		// Balance específico da skill
		SkillBalance bal = SkillBalanceHolder.get(skill.getId());
		if (bal != null)
			damage *= bal.getDouble("powerMul", 1.0);
		
		// ===== MAGICAL CRITICAL =====
		if (Formulas.calcMagicalCritical(attacker, target, skill, profile))
		{
			double mCritMul = (profile != null) ? profile.getDouble("mCritDmgMul", 2.5) : 2.5;
	...
	public static boolean calcMagicalCritical(Creature attacker, Creature target, L2Skill skill, ClassProfile profile)
	{
		// Skills que não podem critar
		if (!skill.isMagic() || skill.isToggle() || skill.isPassive())
			return false;
		
		// Base MCrit do personagem (status + passivos)
		double mCritRate = attacker.getMCriticalHit(target, skill);
		
		// Multiplicador por classe (XML)
		if (profile != null)
			mCritRate *= profile.getDouble("mCritRateMul", 1.0);
		
		// Resistência a crit mágico (se existir no core)
		double mCritRes = target.calcStat(Stats.MAGIC_DEFENCE, 0, null, skill);
		
		mCritRate /= mCritRes;
		
		// Clamp OFF-like
		mCritRate = Math.min(mCritRate, 30.0); // 30% hard cap IL
		
		// Roll final
		return Rnd.get(100) < mCritRate;
	}
	...
	public static boolean calcPhysicalSkillEvasion(Creature target, L2Skill skill)
	{
		...
		double evasion = target.calcStat(Stats.P_SKILL_EVASION, 0, null, skill);
		...
		if (target instanceof Player)
		{
			...
			if (profile != null)
				evasion *= profile.getDouble("evasionMul", 1.0);
		}
		
		return Rnd.get(100) < evasion;
	}
