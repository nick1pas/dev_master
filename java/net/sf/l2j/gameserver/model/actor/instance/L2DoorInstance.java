...
public class L2DoorInstance extends Creature implements IGeoObject
{
	...
	public boolean isAutoAttackable(Creature attacker)
	{
		...
		if (FOSEvent.isDoorAttackable(getDoorId(), attacker) && FOSEvent.isStarted() && FOSEvent.isPlayerParticipant(attacker.getObjectId()))
		{
	...
	public void reduceCurrentHp(double damage, Creature attacker, boolean awake, boolean isDOT, L2Skill skill)
	{
		...
		if (!(_castle != null && _castle.getSiege().isInProgress()) && !FOSEvent.isDoorAttackable(getDoorId(), attacker) && FOSEvent.isPlayerParticipant(attacker.getObjectId()) && FOSEvent.isStarted())
			return;
		
		if(!Config.ENABLE_DOOR_KILL_MAGICS)
		{
			if (!(attacker instanceof L2SiegeSummonInstance) && (getTemplate().getType() == DoorType.WALL))
				return;	
		}
