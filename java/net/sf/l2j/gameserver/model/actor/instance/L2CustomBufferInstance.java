...
public class L2CustomBufferInstance extends L2NpcInstance
{
	...
	public L2CustomBufferInstance(int objectId, NpcTemplate template)
	{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (currentCommand.equalsIgnoreCase("restore"))
		{
			...
			{
				player.sendMessage("No used Restore in Combat!!");
				return;
			}
		...
		{
			player.broadcastPacket(new MagicSkillUse(this, player, 1056, 1, 100, 0));
		...
		else if (currentCommand.startsWith("skill"))
		{
			...
			{
				if (CharEffectList.PROTECTED_BUFF_IDS.contains(skillId))
				{
					// Buff protegido é selecionado, mas não conta para o limite
					if (!skills.contains(skillId))
					{
						skills.add(skillId); // Adiciona o buff à lista de buffs ativos, sem consumir slot
					}
					else
					{
						player.sendMessage("You have add this buffs.");
					}
				}
				else if (skills.size() < player.getMaxBuffCount())
					skills.add(skillId);
				else
