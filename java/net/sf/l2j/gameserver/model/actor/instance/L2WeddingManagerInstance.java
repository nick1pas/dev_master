...
public class L2WeddingManagerInstance extends L2NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		if (command.startsWith("AskWedding"))
		{
			...
			if (st.hasMoreTokens())
			{
				final Player ptarget = L2World.getInstance().getPlayer(st.nextToken());
				if (ptarget == null)
				{
		...
		else if (command.startsWith("Divorce"))
		{
			...
			final Player partner = L2World.getInstance().getPlayer(Couple.getPartnerId(player.getObjectId()));
			if (partner != null)
	...
	private boolean weddingConditions(Player player, Player ptarget)
	{
	...
	public static void justMarried(Player player, Player ptarget)
	{
