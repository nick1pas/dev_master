...
public class L2CubicInstance
{
	...
	public L2CubicInstance(Player owner, int id, int level, int mAtk, int activationtime, int activationchance, int totallifetime, boolean givenByOther)
	{
		_owner = owner;
		_id = id;
		_matk = mAtk;
	...
	public void getCubicTarget()
	{
		try
		{
			...
			// FOS event targeting
			if (FOSEvent.isStarted() && FOSEvent.isPlayerParticipant(_owner.getObjectId()))
			{
				FOSEventTeam enemyTeam = FOSEvent.getParticipantEnemyTeam(_owner.getObjectId());
	...
	//Novo metodo cubic.
	public boolean isInCubicRange(final Creature owner, final Creature target)
	{
		if (owner == null || target == null)
		{
			return false;
