...
public class CharEffectList
{
	...
	public static final Set<Integer> PROTECTED_BUFF_IDS = Set.of(4554, 4553, 4552, 4551);
	/**
	...
	protected void updateEffectIcons()
	{
		...
		if (_buffs != null && !_buffs.isEmpty())
		{
			if(Config.ENABLE_FINAL_BUFF_SLOT)
			{
				  // Ordena os buffs: primeiro os buffs normais, depois os protegidos
			    _buffs.sort((e1, e2) ->
			    {
			        boolean isProtected1 = PROTECTED_BUFF_IDS.contains(e1.getSkill().getId());
			        boolean isProtected2 = PROTECTED_BUFF_IDS.contains(e2.getSkill().getId());
			        if (!isProtected1 && isProtected2)
			            return -1; // Buff normal vem antes
			        if (isProtected1 && !isProtected2)
			            return 1;  // Buff protegido vem depois
			        return 0; // Ordem original para buffs do mesmo tipo
			    });
			}
			for (L2Effect e : _buffs)
			{
		...
		if (ps != null)
		{
			...
			{
				Player summonOwner = ((L2Summon) _owner).getOwner();
