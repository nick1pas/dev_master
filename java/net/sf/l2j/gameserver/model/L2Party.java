...
public class L2Party
{
	...
	public void removePartyMember(Player player, MessageType type)
	{
		...
		for (Creature character : player.getKnownList().getKnownType(Creature.class))
	...
	public void distributeXpAndSp(long xpReward, int spReward, List<Player> rewardedMembers, int topLvl)
	{
	    final List<Player> validMembers = getValidMembers(rewardedMembers, topLvl);

	    // só aplica bônus de party se for o cálculo original
	    if (!Config.ENABLE_FULL_PARTY_XP)
	    {
	    ...
	    for (Player member : rewardedMembers)
	    {
	        ...
	        if (validMembers.contains(member))
	        {
	            ...
	            if (Config.ENABLE_FULL_PARTY_XP)
	            {
	                // dá o XP/SP base igual pra todos (sem multiplicar pelo tamanho da party)
	                xp = Math.round(xpReward * (1 - penalty));
	                sp = Math.round(spReward * (1 - penalty));
	            }
	            else
	            {
	                // Cálculo original proporcional
	                final double sqLevel = member.getLevel() * member.getLevel();
	                final double preCalculation = (sqLevel / sqLevelSum) * (1 - penalty);

	                xp = Math.round(xpReward * preCalculation);
	                sp = (int) (spReward * preCalculation);
	            }

	            member.updateKarmaLoss(xp);
	            member.addExpAndSp(xp, sp);
	        }
	        else
	        {
