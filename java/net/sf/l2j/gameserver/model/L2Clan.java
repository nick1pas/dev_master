...
public class L2Clan
{
	...
	public L2Clan(int clanId, int leaderId)
	{
		...
		loadRankingPoints();
		initializePrivs();
		ClanRankingManager.getInstance().addClanRanking(_clanId, _champPoints + _bossPoints + _siegePoints);
	...
	public void addRankingBossPoints(int points) 
	{
		loadRankingPoints();
		_bossPoints += points;
		storeRankingPoints();
	...
	public void addRankingSiegePoints(int points)
	{
		loadRankingPoints();
		_siegePoints += points;
		storeRankingPoints();
	}
	protected void loadRankingPoints()
	{
		try (Connection con = ConnectionPool.getConnection()) 
		{
			PreparedStatement statment = con.prepareStatement("SELECT * FROM clan_points WHERE clan_id=?");
			statment.setInt(1, _clanId);
			ResultSet result = statment.executeQuery();
			boolean hasRecord = result.next();
			
			if (hasRecord)
			{
				_champPoints = result.getInt("champ_points");
				_bossPoints = result.getInt("boss_points");
				_siegePoints = result.getInt("siege_points");
			} 
			...
			if (!hasRecord) 
			{
				...
				_bossPoints = 0;
				_siegePoints = 0;
				createPointsRecord(_clanId);
			} 
		} 
		catch (Exception e) 
		{
			_log.warning("error while loading clan points from db " + e);
		} 
	}

	private void storeRankingPoints() 
	{
		try (Connection con = ConnectionPool.getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE clan_points SET champ_points=?, boss_points=?, siege_points=? WHERE clan_id=?");
			statement.setInt(1, _champPoints);
			statement.setInt(2, _bossPoints);
			statement.setInt(3, _siegePoints);
			statement.setInt(4, _clanId);
			statement.execute();
			statement.close();
		} 
		catch (Exception e) 
		{
			_log.warning("Error While saving Ranking Points for Clan " + getName());
		} 
		ClanRankingManager.getInstance().addClanRanking(_clanId, _champPoints + _bossPoints + _siegePoints);
	}
	private void createPointsRecord(int clan_id)
	{
		try (Connection con = ConnectionPool.getConnection())
		{
			PreparedStatement checkStatement = con.prepareStatement("SELECT * FROM clan_points WHERE clan_id=?");
			checkStatement.setInt(1, _clanId);
			...
			PreparedStatement pointsStatment = con.prepareStatement("INSERT INTO clan_points(clan_id, champ_points, boss_points, siege_points) VALUES(?,?,?,?)");
			pointsStatment.setInt(1, _clanId);
			pointsStatment.setInt(2, 0);
			pointsStatment.setInt(3, 0);
			pointsStatment.setInt(4, 0);
			pointsStatment.execute();
			pointsStatment.close();
		} 
		catch (Exception e)
		{
			_log.warning("error while saving new clan points to db " + e);
		} 
	}
	...
	public void removeClanMember(int objectId, long clanJoinExpiryTime)
	{
		...
		if (exMember.isOnline())
		{
			...
			if (player.isClanLeader())
			{
				...
				//player.setClanCreateExpiryTime(System.currentTimeMillis() + Config.ALT_CLAN_CREATE_DAYS * 86400000L);
				  player.setClanCreateExpiryTime(System.currentTimeMillis() + Config.ALT_CLAN_CREATE_DAYS * 60 * 1000L);
			}
		...
		else
		{
			try (Connection con = ConnectionPool.getConnection())
			{
				...
				statement.setLong(3, _leader.getObjectId() == objectId ? System.currentTimeMillis() + Config.ALT_CLAN_CREATE_DAYS * 60 * 1000L : 0);
	...
	public void dissolveAlly(Player player)
	{
		...
		setAllyPenaltyExpiryTime(currentTime + Config.ALT_CREATE_ALLY_DAYS_WHEN_DISSOLVED * 60 * 1000L, L2Clan.PENALTY_TYPE_DISSOLVE_ALLY);
		updateClanInDB();
