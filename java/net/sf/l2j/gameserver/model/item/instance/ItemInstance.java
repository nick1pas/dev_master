...
public final class ItemInstance extends L2Object implements Runnable
{
	...
	// private static final long RAID_LOOT_PROTECTION_TIME = 15000;
	private static final long REGULAR_LOOT_PROTECTION_TIME = Config.TIME_PICKUP_NORMAL * 1000;
	private static final long RAID_LOOT_PROTECTION_TIME = Config.TIME_PICKUP_BOSS * 1000;
	...
	public void changeCountWithoutTrace(int count, Player creator, L2Object reference)
	{
	...
	private void updateItemAttributes(Connection pooledCon)
	{
		if (pooledCon != null)
		{
			try (PreparedStatement statement = pooledCon.prepareStatement("REPLACE INTO augmentations VALUES(?,?,?,?)"))
			{
				statement.setInt(1, getObjectId());
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement statement = con.prepareStatement("REPLACE INTO augmentations VALUES(?,?,?,?)"))
		{
			statement.setInt(1, getObjectId());
	...
	{
		_item = ItemTable.getInstance().getTemplate(getItemId());
	}
	
	private volatile int _pickupReservedBy = 0;
	
	public boolean tryReservePickup(int objectId)
	{
		synchronized (this)
		{
			if (_pickupReservedBy != 0)
				return false;
			
			_pickupReservedBy = objectId;
			return true;
		}
	}
	
	public void releasePickupReservation(int objectId)
	{
		synchronized (this)
		{
			if (_pickupReservedBy == objectId)
				_pickupReservedBy = 0;
		}
	}
