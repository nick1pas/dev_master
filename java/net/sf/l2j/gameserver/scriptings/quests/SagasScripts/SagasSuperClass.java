package net.sf.l2j.gameserver.scriptings.quests.SagasScripts;
...
public class SagasSuperClass extends Quest
{
	...
	private static void cast(L2Npc npc, Creature target, int skillId, int level)
	{
	...
	public QuestState findRightState(L2Npc npc)
	{
		if (_SpawnList.containsKey(npc))
		{
			final Player player = L2World.getInstance().getPlayer(_SpawnList.get(npc));
			if (player != null)
	...
	@Override
	public String onSkillSee(L2Npc npc, Player player, L2Skill skill, L2Object[] targets, boolean isPet)
	{
		if (_SpawnList.containsKey(npc) && _SpawnList.get(npc) != player.getObjectId())
		{
			Player quest_player = L2World.getInstance().getPlayer(_SpawnList.get(npc));
