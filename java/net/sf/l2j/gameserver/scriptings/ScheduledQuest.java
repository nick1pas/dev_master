...
public abstract class ScheduledQuest extends Quest
{
	...
	private final void print(Calendar c)
	{
		...
		_log.info(
			getName() +
			(c == _start ? ": Next start = " : ": Next end = ") +
			String.format("%d.%d.%d %d:%02d:%02d", c.get(Calendar.DAY_OF_MONTH), c.get(Calendar.MONTH) + 1, c.get(Calendar.YEAR), c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), c.get(Calendar.SECOND))
		);
	}
}
