...
{
	private static final L2RaidZone AQ_LAIR = ZoneManager.getInstance().getZoneById(110017, L2RaidZone.class);
	...
	public QueenAnt()
	{
		...
		if (Config.WAIT_TIME_FOR_SPAWN_ALL_NPCS)
	    {
	        ...
	        if (now.isBefore(spawnTime))
	        {
	            ...
	            startQuestTimer("queen_unlock", delay, null, null, false);
		...
		if (GrandBossManager.getInstance().getBossStatus(QUEEN) == DEAD)
		{
			...
			{
				L2GrandBossInstance queen = (L2GrandBossInstance) addSpawn(QUEEN, -21615, 182400, -5725, 0, false, 0, false);
