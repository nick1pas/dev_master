...
public class PrimevalIsle extends L2AttackableAIScript
{
	...
	public PrimevalIsle()
	{
		...
		for (L2Spawn npc : SpawnTable.getInstance().getSpawnTable())
			if (Util.contains(MOBIDS, npc.getNpcId()) && npc.getNpc() != null && npc.getNpc() instanceof Attackable)
	...
	public String onAdvEvent(String event, L2Npc npc, Player player)
	{
		...
		if (event.equalsIgnoreCase("skill"))
		{
			...
			for (Playable playable : npc.getKnownList().getKnownTypeInRadius(Playable.class, npc.getTemplate().getAggroRange()))
			{
				if (!playable.isDead())
					playableCounter++;
			}
	...
	public String onAttack(L2Npc npc, Player attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		for (Attackable called : attacker.getKnownList().getKnownTypeInRadius(Attackable.class, 2000))
		{
