package net.sf.l2j.gameserver.scriptings.scripts.ai;
...
public class L2AttackableAIScript extends Quest
{
	...
	protected void registerNpcs()
	{
		...
		for (NpcTemplate template : NpcTable.getInstance().getAllNpcs())
		{
			try
			{
				if (Attackable.class.isAssignableFrom(Class.forName("net.sf.l2j.gameserver.model.actor.instance." + template.getType() + "Instance")))
				{
	 ...
	 */
	public static Player getRandomPlayer(L2Npc npc)
	{
		List<Player> result = new ArrayList<>();
		
		for (Player player : npc.getKnownList().getKnownType(Player.class))
		{
			if (player.isDead())
				continue;
