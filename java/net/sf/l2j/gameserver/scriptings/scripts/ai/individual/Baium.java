	...
	public Baium()
	{
		...
		if (Config.WAIT_TIME_FOR_SPAWN_ALL_NPCS)
	    {
	        ...
	        if (now.isBefore(spawnTime))
	        {
	            ...
	            startQuestTimer("baium_unlock", delay, null, null, false);
...
//	}
	 ...
	 */
	private static Creature getRandomTarget(L2Npc npc)
	{
		int npcId = npc.getNpcId();
		List<Creature> result = new ArrayList<>();
		
		for (Creature obj : npc.getKnownList().getKnownType(Creature.class))
		{
		...
		if (result.isEmpty())
		{
			...
			{
				  return null; // Não retorna nenhum alvo.
			}
		}
