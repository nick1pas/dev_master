...
public class Zaken extends L2AttackableAIScript
{
	...
	private static final L2RaidZone _zakenLair = ZoneManager.getInstance().getZoneById(110000, L2RaidZone.class);
	private int _1001 = 0;
	private int _ai0 = 0;
	private int _ai1 = 0;
	private int _ai2 = 0;
	...
	public Zaken()
	{
		...
		if (Config.WAIT_TIME_FOR_SPAWN_ALL_NPCS)
	    {
	        ...
	        if (now.isBefore(spawnTime))
	        {
	            ...
	            startQuestTimer("zaken_unlock", delay, null, null, false);
		...
		if (status == DEAD)
		{
			...
			{
		//		final L2GrandBossInstance zaken = (L2GrandBossInstance) addSpawn(ZAKEN, 55276, 219176, -3227, 0, false, 0,false);
				final L2GrandBossInstance zaken = (L2GrandBossInstance) addSpawn(ZAKEN, Config.CUSTOM_SPAWN_ZAKEN[0], Config.CUSTOM_SPAWN_ZAKEN[1], Config.CUSTOM_SPAWN_ZAKEN[2], 0, false, 0,false);
				GrandBossManager.getInstance().setBossStatus(ZAKEN, ALIVE);
				spawnBoss(zaken);
				/*if (Config.ANNOUNCE_BOSS_ALIVE)
				{
					Broadcast.gameAnnounceToOnlinePlayers("Epic Boss: " + zaken.getName() + " spawned in world.");
				}*/
			}
		}
		else
	...
	public void spawnBoss(L2GrandBossInstance npc)
	{
		...
		npc.broadcastPacket(new PlaySound("BS01_A"));
		_ai0 = 0;
		_ai1 = npc.getX();
		_ai2 = npc.getY();
		_ai3 = npc.getZ();
	...
	public String onAttack(L2Npc npc, Player attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		if (npcId == ZAKEN)
		{
			...
			if (Rnd.get(10) < 1)
			{
				...
				{
					for (Creature character : npc.getKnownList().getKnownTypeInRadius(Creature.class, 100))
					{
