...
public class GeoEngine
{
	...
	private static final long LOS_CACHE_TIME = 1500; // 1.5 segundos
	
	private final ConcurrentHashMap<Integer, LosCache> _losCache = new ConcurrentHashMap<>();
	
	private static class LosCache
	{
		boolean result;
		long time;
		int tx, ty, tz;
	}
	private static final int LOS_MIN_DISTANCE = 200;

	public final boolean canSeeTarget(L2Object origin, L2Object target)
	{
		// quick distance check (não precisa de LOS se perto)
		if (origin.distance3DSq(target) < (LOS_MIN_DISTANCE * LOS_MIN_DISTANCE))

			return true;
		
		// verifica cache
		LosCache cache = _losCache.get(target.getObjectId());
		if (cache != null)
		{
			if (System.currentTimeMillis() - cache.time < LOS_CACHE_TIME)
			{
				// se target não mudou posição, reutiliza
				if (cache.tx == target.getX() && cache.ty == target.getY() && cache.tz == target.getZ())
					return cache.result;
			}
		}
		...
		{
			double oheight = (origin instanceof Creature) ? ((Creature) origin).getCollisionHeight() * 2 : 0;
			double theight = (target instanceof Creature) ? ((Creature) target).getCollisionHeight() * 2 : 0;
			
			result = door ? checkSeeOriginal(gox, goy, goz, oheight, gtx, gty, gtz, theight) : checkSee(gox, goy, goz, oheight, gtx, gty, gtz, theight);
		}
		
		// salva cache
		LosCache newCache = new LosCache();
		...
		_losCache.put(target.getObjectId(), newCache);
	 ...
	 * @param gty : target Y geodata coordinate
	 * @param gtz : target Z geodata coordinate
	 * @param theight : target height (if instance of {@link Creature} or {@link L2DoorInstance})
	 * @return {@code boolean} : True, when target can be seen.
	 */
