...
public class SevenSignsFestival
{
	...
	{
		Player player = L2World.getInstance().getPlayer(partyMemberName);
		if (player != null)
		{
		...
		if (festivalId > -1)
		{
			if (_festivalInitialized)
			{
				...
				if (festivalParty == null)// leader has left
					...
					{
						Player partyMember = L2World.getInstance().getPlayer(partyMemberObjId);
						if (partyMember == null)
							continue;
	...
	private class L2DarknessFestival
	{
		...
		protected void festivalInit()
		{
			...
			if (_participants != null && !_participants.isEmpty())
			{
				...
				{
					Player participant = L2World.getInstance().getPlayer(participantObjId);
					if (participant == null)
						continue;
