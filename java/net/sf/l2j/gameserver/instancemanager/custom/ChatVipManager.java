...
public class ChatVipManager
{
	private static final Logger _log = Logger.getLogger(ChatVipManager.class.getName());
	
	private final Map<Integer, Long> _chats;
	protected final Map<Integer, Long> _chatsTask;
	private ScheduledFuture<?> _scheduler;
	
	public static ChatVipManager getInstance()
	{
		return SingletonHolder._instance;
	}
	
	protected ChatVipManager()
	{
	...
	public void load()
	{
		...
		{
			PreparedStatement statement = con.prepareStatement("SELECT objectId, duration FROM chat_vip_manager ORDER BY objectId");
		...
		_log.info("ChatVIP_Manager: Loaded " + _chats.size() + " characters with chat time.");
	}
	...
	public void addChatTime(int objectId, long duration)
	{
		...
		try (Connection con = ConnectionPool.getConnection())
		{
			PreparedStatement statement = con.prepareStatement("INSERT INTO chat_vip_manager (objectId, duration) VALUES (?, ?)");
	...
	public void updateChatTime(int objectId, long duration)
	{
		...
		try (Connection con = ConnectionPool.getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE chat_vip_manager SET duration = ? WHERE objectId = ?");
	...
	public void removeChatTime(int objectId)
	{
		...
		try (Connection con = ConnectionPool.getConnection())
		{
			PreparedStatement statement = con.prepareStatement("DELETE FROM chat_vip_manager WHERE objectId = ?");
			statement.setInt(1, objectId);
			statement.execute();
			statement.close();
		}
		...
		public final void run()
		{
			...
			for (Map.Entry<Integer, Long> entry : _chatsTask.entrySet())
			{
				...
				if (System.currentTimeMillis() > duration)
				{
					...
					player.sendMessage("VIP chat available.");
				}
			}
		}
	}
	...
	{
		protected static final ChatVipManager _instance = new ChatVipManager();
	}
}